...
- **Load commands carry a `LoadIntent`** — claimed when the intent starts (before the awaits), not before the send; the decoder drops any load older than one already delivered. [→](docs/architecture/invariants.md#every-load-command-carries-its-intent)
