...
- `crates/core/` (`waveflow-core`) — portable business logic, designed to be reusable from the future `waveflow-server`. Holds the domain DTOs, repository traits + SQLite impls, scanner helpers + upserts, smart-playlist engine, audio analysis, DSD format conversion, HTTP clients (Deezer / Last.fm / LRCLIB), artwork pipeline. **Zero Tauri / `cpal`** dependencies. Full split rules in [`docs/architecture/crates.md`](docs/architecture/crates.md).
