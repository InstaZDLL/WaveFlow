...
- **`dlna/`** (axum + SSDP, opt-in) · **`mpd/`** (TCP MPD protocol, opt-in) · **`media_controls.rs`** (souvlaki → SMTC / MPRIS / MediaRemote) · **`discord_presence.rs`** · **`queue.rs`** · **`player_actions.rs`** (shared control sequence) · **`remote/`** (remote source + sync v2, feature `sync_v2`, off by default; `mod sync` is now a permanent no-op stub — v1 was removed in the RFC-005 cutover) · **`backup.rs`** · **`db/`** (pool wiring + `migration_heal`).
