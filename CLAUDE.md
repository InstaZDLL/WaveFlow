...
- **Playlist `tracks` insert ops carry a `snapshots` map** — omitting it silently breaks the server's public share preview. [→](docs/architecture/invariants.md#outbound-playlist--field-tracks-ops-carry-a-snapshot-map)
- **Track sync emit** — `entity_id` is the _file_path_, `added_at` keeps the original import time on re-emit, folder removal emits per row (no server cascade), and every enqueue calls `state.drain.notify()`. [→](docs/architecture/invariants.md#track-sync-emit)
