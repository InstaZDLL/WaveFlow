 ...
 */
/**
 * Resolve a server cover by hash, with the shared cache, the shared in-flight
 * deduplication and the one-shot retry.
 *
 * Exported because not every surface renders a plain `<img>`: the artist grid
 * feeds its result to `FadeInImage`, which fades over a letter placeholder.
 * `onError` / `onLoad` are what drive the retry, so a consumer that cannot
 * wire them degrades to whatever its own placeholder is — which for the artist
 * grid is exactly the fallback it already had.
 */
export function useRemoteArtworkSrc(hash: string | null): {
  src: string | null;
  onError: () => void;
  onLoad: () => void;
} {
