...
- **A plugin that cannot load says so, instead of going quiet.** The host runs WASM **components**; a core module (`cargo build` instead of `cargo component build`) is valid wasm in the wrong format, and before the gate existed such a plugin installed cleanly and then did nothing, with only a `warn!` to show for it. [`plugin::binfmt`](../../src-tauri/crates/core/src/plugin/binfmt.rs) tells the two apart from the preamble; the store refuses a non-component at install, `load_plugin` records why a load failed, and the plugin list renders it as `PluginInfo.failure`. Log and surface these errors through `detail()`, never `%err` — wasmtime's outermost message says "failed to parse WebAssembly module" and the layer that names the cause is below it. See [plugins § when a plugin cannot run](../features/plugins.md#when-a-plugin-cannot-run).
