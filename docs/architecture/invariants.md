...
The tray menu, the OS media keys, the Windows taskbar thumbnail buttons and the MPD server all need the same sequence the frontend gets from `commands::player`: advance the queue → `emit_track_changed` → `emit_queue_changed` → hand the track to the decoder.
...
- **Say it before the event loop starts.** `setup` runs from _inside_ the loop (`RuntimeRunEvent::Ready`), so nothing queued from there is dispatched until it returns — and a fatal path never returns. One failure per platform. Two are measured: on Linux `rfd`'s GTK backend hands every dialog to a global thread that iterates the _default_ GLib main context, which `tao` owns on the main thread we would be blocking, so no window is ever created and the process hangs (#529); on Windows `TaskDialogIndirect` creates its window and never shows it (`WS_VISIBLE` stays clear, #528). The third is **inferred, never run** — read out of `rfd` 0.16's sources, macOS should fail twice over, since `run_on_main` dispatches to a main queue nobody is draining and panics outright before `NSApplication` is running. There is no macOS job in CI, so treat it as the reason the design avoids the situation rather than as an observation. What _is_ observed, on all three, is the shape that replaced it: the pre-flight dialog was watched appearing on Fedora 44 (GNOME/Wayland) and on macOS 26.6.1, and on Windows in #528 — each time blocking until dismissed, then exiting 1 with the fatal line already flushed to the log file. So the inference above is about the road not taken, not about whether this works. So the explanation moved to [`preflight`](../../src-tauri/crates/app/src/db/schema_guard.rs), called from `run` before `tauri::Builder` is even constructed — no main context held, no splash to hide. The guard inside `setup` stays as a backstop and may only **exit**, never present. One Linux-only precondition survives: `rfd` cannot report a failed `gtk_init_check` — `run_blocking` just parks on a condvar forever — so the dialog is skipped outright when neither `DISPLAY` nor `WAYLAND_DISPLAY` is set. Text is English like the tray's seed labels: the language preference lives in the database we just refused to open.
- **The pre-flight may not create, and may not refuse on its own doubt.** It opens with `create_if_missing(false)` — materializing `app.db` would turn "no install yet" into "install with an empty database" and the first-run path would never run again — and anything short of a verdict (no database, a file it can't read, a query that fails) proceeds to normal startup, where the real guards get the last word. It vets exactly the two databases startup will open, `app.db` and the profile [`resolve_target_profile`](../../src-tauri/crates/app/src/state.rs) picks, sharing that lookup with `bootstrap` rather than re-deriving it: one profile left behind by a newer build is no reason to ground a launch that won't touch it.
...
records the order preparations _finished_ — which is exactly the order that
...
The scan's own toast now shows only the _outcome_ — "412 added, 3 errors" — and leaves live progress to the bar: two surfaces counting the same files is the inconsistency this was written to remove, but a task row vanishes when its task ends, so the summary needs somewhere else to live.
...
- **A plugin that cannot load says so, instead of going quiet.** The host runs WASM **components**; a core module is valid wasm in the wrong format, and before the gate existed such a plugin installed cleanly and then did nothing, with only a `warn!` to show for it. It reaches a release more easily than it sounds: the two that did were built with the right command and packaged with the wrong file. [`plugin::binfmt`](../../src-tauri/crates/core/src/plugin/binfmt.rs) tells the two apart from the preamble; the store refuses a non-component at install, `load_plugin` records why a load failed, and the plugin list renders it as `PluginInfo.failure`. Log and surface these errors through `detail()`, never `%err` — wasmtime's outermost message says "failed to parse WebAssembly module" and the layer that names the cause is below it. See [plugins § when a plugin cannot run](../features/plugins.md#when-a-plugin-cannot-run).
