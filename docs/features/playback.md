...
- **The user had paused it** (#611). `notify_device_lost` has turned a playing session into `Paused` by then, so the decision reads `paused_output`, which on the playback path only the decoder's own pause raises. Resuming these started music on whatever device the system fell back to.
