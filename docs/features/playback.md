...
Spawn-first is the order we want everywhere else: a failed open then costs nothing, because the stream the user is listening to is still installed and still playing. Releasing first is safe in the macOS case anyway, since `spawn_output_with_mode` falls back to shared mode on its own, so the caller still comes back holding a stream.
