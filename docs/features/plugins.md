...
- **Manual motion cover** (issue #408): a user can set a local mp4 per album via the "Set motion cover" button (Film icon) on the album page → [`MotionCoverPickerModal`](../../src/components/common/MotionCoverPickerModal.tsx). It stores into a never-evicted `motion/` dir and takes precedence over any plugin resolution. Each file is capped at 256 MiB — raised from 64 MiB together with the plugin-download cap, because a 4K cover does not fit under a ceiling sized for Apple's 1080 renditions. The directory itself has no aggregate ceiling; nothing evicts it. **The override is keyed by album id, so the playing track must carry one.** Until #766 it did not: `QueueTrack` (the engine's playing-track shape) selected `al.title` but not `t.album_id`, and [`queuePayloadToTrack`](../../src/lib/queueTrack.ts) set `album_id: null`. Every now-playing surface therefore asked by title alone, the backend skipped the manual row and went straight to the plugins, and a hand-set cover never showed on any platform — "removing" it seemed to do nothing (the plugin's was the one on screen), and disabling the plugin seemed to disable manual covers too. The three queue queries now select `t.album_id` and the payload carries it, which also gives the immersive view and the queue panel their go-to-album entry. A cover that fails to load still falls back to the static one, but now says so in the console. **Settings → Immersive view → Show animated covers** ([`useMotionCovers`](../../src/hooks/useMotionCovers.ts), `profile_setting` `ui.motion_covers`, default on) hides every motion cover, hand-set ones included, and stops the plugin fan-out: `useAlbumMotionArtwork` reads it, so every surface obeys it at once. Before it, disabling the plugin was the only off switch, and it could not hide a hand-set cover. It is a setting and not a player button on purpose — the immersive top bar is already full on a small screen.
- **Opt-in local cache** (default **OFF**, `app_setting['motion_artwork.cache_enabled']`): when on, the resolved plugin mp4 is downloaded into an app-wide LRU cache (`motion_cache/` under the cache root — `<app-data>/waveflow/` by default, or wherever _Settings → Storage and backups → Folders and caches_ moved it; hash-addressed by source URL, 1 GB cap, mtime-based eviction) and served from disk. OFF relies on the webview's transient HTTP cache. Toggle + size + clear live in **Settings → Storage and backups → Folders and caches** ([`MediaCachesCard`](../../src/components/views/settings/MediaCachesCard.tsx)); i18n under `settings.motionArtwork.*`.
...
- **Verified installs** — `plugin.wasm` blake3 is pinned by the trusted registry, checked before a stage-swap; a tampered release fails. The binary must also _be_ a component, or the install is refused rather than landing a plugin that can never load.
