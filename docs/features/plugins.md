...
By default the plugin returns a remote URL the webview streams; an **opt-in local cache** (`app_setting['canvas.cache_enabled']`, default OFF) downloads the resolved mp4 into an app-wide LRU (`canvas_cache/` under the cache root — `<app-data>/waveflow/` by default, or wherever *Settings → Data → Cache location* moved it) and serves the on-disk copy — it reuses the motion-artwork cache's `motion_cache` primitives (download + eviction + SSRF guard) with a separate dir, and its toggle + footprint + clear live in **Settings → Data**, beside the motion-cache control and right under the cache-location card that decides which drive both caches sit on ([`MediaCachesCard`](../../src/components/views/settings/MediaCachesCard.tsx), one shared `LocalCacheOption` per cache).
...
- **Opt-in local cache** (default **OFF**, `app_setting['motion_artwork.cache_enabled']`): when on, the resolved plugin mp4 is downloaded into an app-wide LRU cache (`motion_cache/` under the cache root — `<app-data>/waveflow/` by default, or wherever *Settings → Data → Cache location* moved it; hash-addressed by source URL, 1 GB cap, mtime-based eviction) and served from disk. OFF relies on the webview's transient HTTP cache. Toggle + size + clear live in **Settings → Data** ([`MediaCachesCard`](../../src/components/views/settings/MediaCachesCard.tsx)); i18n under `settings.motionArtwork.*`.
