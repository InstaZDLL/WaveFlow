...
- `immersive.merged_lyrics` ON + window ≥ 900 px → **merged**: now-playing left (~60%) + tabbed control panel right (~40%). The top-bar `PanelRight` toggle hides/shows the panel (left re-centres when hidden); the active tab (Lyrics / Queue) persists across hide/show. A clean lyrics miss keeps a discreet "No lyrics — Import / Fetch" CTA so the per-track relayout stays calm. The view's own buttons float over the top right of that panel, and with the Canvas ones they take about 330 px. The tabs sit on the **same line**, as round icon buttons in the same style (`p-2.5`, a 22 px icon, the tab's name as its tooltip), about 120 px wide, so both fit down to a 32rem panel; only narrower than that does the tab header step below the buttons — a container query on the panel, since its width is a share of the window's. The tabs used to be a labelled pill of about 250 px, which ran under the buttons in any panel narrower than 48rem, a 1080p window (#733); dropping it below them fixed the overlap but pushed the lyrics column, and the focused line it centres, well down the screen (#750). The column centres its active line in its own scroller, so every pixel of header is half a pixel of drift.
