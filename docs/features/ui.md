...
- **Volume is engine-wide, not per-window** — one engine behind two `PlayerContext`s, so [`player_set_volume`](../../src-tauri/crates/app/src/commands/player.rs) emits `player:volume-changed` and every window adopts it. The window that just touched its own slider ignores the echo for 500 ms: the push is debounced, so an echo can carry a value the user has already dragged past. (Repeat / shuffle have no such event — `player:options-changed` only fires for external surfaces like MPD — so those two still drift between the windows.)
