...
- `immersive.merged_lyrics` ON + window ≥ 900 px → **merged**: now-playing left (~60%) + tabbed control panel right (~40%). The top-bar `PanelRight` toggle hides/shows the panel (left re-centres when hidden); the active tab (Lyrics / Queue) persists across hide/show. A clean lyrics miss keeps a discreet "No lyrics — Import / Fetch" CTA so the per-track relayout stays calm. The view's own buttons float over the top right of that panel, and with the Canvas ones they take about 330 px; in a panel narrower than 48rem (a 1080p window not maximised) the tab header drops below them — a container query on the panel, since its width is a share of the window's (#733).
