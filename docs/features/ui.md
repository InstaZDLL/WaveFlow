...
index rather than re-deriving a view. What a page needs to be _restored_ travels
...
- **A scan's safe stopping point is not between two files.** The scan ends with
  ...
  _because the user pressed stop_. The sweep is skipped entirely on a cancelled
...
- **The desktop lyrics window is left alone.** It already has its own text and sung colours, and a richer style than one value can carry.
...
- `immersive.merged_lyrics` ON + window ≥ 900 px → **merged**: now-playing left (~60%) + tabbed control panel right (~40%). The top-bar `PanelRight` toggle hides/shows the panel (left re-centres when hidden); the active tab (Lyrics / Queue) persists across hide/show. A clean lyrics miss keeps a discreet "No lyrics — Import / Fetch" CTA so the per-track relayout stays calm. The view's own buttons float over the top right of that panel, and the tabs share their line. **Where they go is measured, not guessed**: the panel compares the room between its left edge and the first floating button with the labelled pill's width (read from a hidden copy, so it follows the language) and shows the **pill** when it fits, **round icon buttons** (the buttons' own style, names as tooltips) when only they do, and the pill **below** the buttons only when not even the icons fit. A `ResizeObserver` re-measures as the window resizes, as the Canvas buttons come and go with the track, and as the language changes. Nothing it measures depends on the layout it picks, so it cannot oscillate. It replaces a 48rem container breakpoint (#733) that dropped the pill below the buttons in panels that had room for it — a fixed breakpoint cannot know how many buttons there are or how long the labels are — and that drop pushed the lyrics column, and the focused line it centres, well down the screen (#750). The column centres its active line in its own scroller, so every pixel of header is half a pixel of drift.
...
**Where the clip is kept (#695)** — a hand-set Canvas normally lives in the profile's never-evicted `canvas/` directory; with **Settings → Storage and backups → Folders and caches → Keep clips with the music** on, a new one is written into `<library folder>/.waveflow/canvas/` instead, so it travels with the album and sits on the drive the music is on. Reading looks in both, the library first, because both are hash-addressed and the disk is a better authority than a column — so the switch moves nothing and there is no migration to half-finish. A folder that refuses the write keeps the clip in the app directory rather than losing it. Same treatment for a hand-set [motion cover](plugins.md#apple-motion-artwork-metadata-world); the plugin caches stay in the app folder, since they are LRU and evicting out of somebody's music folder is not a behaviour worth having. Full layout in [`storage.md`](../architecture/storage.md#inside-the-users-library-issue-695).
...
- **macOS keeps the system's buttons.** Dropping the decorations would take the traffic lights with them, and drawing our own is the one thing a macOS user would call _not_ native. An overlay title bar still paints the window title over our content and `hiddenTitle` is creation-time only, so the title is what we empty — and restore with the frame; the app is named by the menu bar either way. The overlay style also provides no drag region of its own, which is what the 28 px strip is for.
- **Applied before the reveal.** The main window is created hidden ([splash handoff](#surviving-a-renderer-that-cannot-paint-595)), and `restore_bounds_and_reveal` puts the chrome on just before showing it, so nobody watches the frame they turned off appear and vanish. `set_window_chrome` applies _and then_ persists — in that order, so a frame that refused to change is never recorded as the one in use, and on macOS because the style and the title are two calls that must move together. If the write itself fails, the frame is put back: the caller is told the change failed and has to be able to believe it.
- **No bare window between the reveal and the bar.** The frame comes off before the window is shown, while the frontend's first answer is an IPC round-trip away — so a Linux user who chose `app` would see a frameless rectangle with no title bar yet for a few frames. [`useWindowChrome`](../../src/hooks/useWindowChrome.ts) seeds its first render from a `localStorage` cache, the same bargain the theme, skin and contrast bootstraps make, and validates what it reads back (page-writable storage, and a bad value would draw a title bar over a framed window). Deliberately _not_ done by gating the `app://ready` signal on the read: that path is what reveals the window at all, and a read that never resolved would leave the user on the splash — a far worse failure than a flash ([#626](#splash-screen)).
- **What is reported is what the window accepted.** A failure at startup logs and sets a session flag; `get_window_chrome` then answers `system` while leaving the stored preference alone. The flag is cleared inside `apply_window_chrome` itself, on any success — the startup path retries up to ten times, so clearing it at one caller would leave a failed first attempt describing a window a later attempt had already changed. Without it the interface would draw a title bar over the system one the window in fact still has — two title bars, from a preference nobody could honour. The flag clears as soon as a `set_window_chrome` succeeds.
...
- **Appearance** — `profile_setting['ui.desktop_lyrics_style']` (JSON: font size 20–72 px, text colour, sung colour, outline, background opacity 0–80 %, show translation, show next line — off leaves a single line, since the second row, when there is no translation, is the next line and a tester read it as a translation that would not switch off, #735) through [`useDesktopLyricsStyle`](../../src/hooks/useDesktopLyricsStyle.ts). The outline is eight `text-shadow`s rather than `-webkit-text-stroke`, which eats into thin glyphs and needs `paint-order` that HTML text does not honour everywhere.
  - **Cross-window sync.** `useProfileSetting` broadcasts a window event, which never leaves its document, and this setting is edited in the main window while it is read in the overlay. A write is broadcast app-wide as the backend Tauri event `profile-setting:changed`, and [`profileSettingBridge`](../../src/lib/profileSettingBridge.ts) turns it back into the per-document window event in every other webview, dropping the writer's own echo by window label. This hook carried a private copy of that machinery until #741 made it general (#743) — the writer is skipped because it already painted the value optimistically, and because while a slider is dragged a re-read landing between writes would put the older stored value back on screen.
- **Resizing** — the window is undecorated and a press anywhere on it starts a move (`startDragging`), which left nothing to grab its sides with: it could not be widened, and a long line was cut (#735). A press within 8 px of an edge or corner calls `startResizeDragging` in that direction instead, with the matching resize cursor while unlocked (`core:window:allow-start-resize-dragging`).
...
The same signal answers a second question. When GPU-accelerated rendering fails, the app starts and shows nothing — the window exists, the process is alive, and there is a blank rectangle with no message and no way back. So a launch **arms a marker** before the window is created and disarms it on that first committed render; a launch that finds the marker still armed knows the previous one never got that far and starts in software rendering instead. [`render_mode`](../../src-tauri/crates/app/src/render_mode.rs) holds it, called from `run` before `tauri::Builder` is constructed — like the schema guard, and for a sharper reason: the variables it sets are read by the web engine when _its_ process starts, and the `main` webview is created by `Builder::build`, before `setup` runs at all.
...
**The fallback sticks**, because a one-shot one would be worse than none: the software launch paints, the marker clears, and the launch after it is blank again — every other start. A software launch that painted records that it was software, and Settings → Maintenance → Diagnostics offers the way back ("Try the GPU again", applied on the next start). The exception is software _also_ failing to paint: the GPU was then not the problem, so that returns to the default rather than degrading rendering for a fault it does not address.
...
And the other direction: **the second launch reads the first one's marker.** Two launches a few milliseconds apart — a double click — both decide before the plugin sorts them out, and the second found the first's marker armed only because the first had not painted *yet* (#755). When the second was the one that stayed, it started in software and, having painted there, remembered it for every launch after. The marker now records the process that armed it (`armed_by`), and one armed by a WaveFlow launch that is still running — checked by executable name, so a reused pid does not count — is set aside. A marker whose process is gone keeps its meaning, as does one written before the field existed.
...
- **Import:** always allocates a fresh profile row — never overwrites — under the archive's profile name, or `Name (2)`, `Name (3)`… when a profile already holds it (#767): `profile.name` is UNIQUE and restoring a backup of a profile that still exists is the ordinary case, so the name is taken by construction. It retries on the violation itself rather than checking first. The command returns the new id **and** that name, the success message shows the name, and Settings refreshes the profile list so the selector it points at actually lists the profile. It then extracts the archive under `profiles/<new_id>/`. Failures roll the row back so a half-imported profile doesn't survive the error. Before the sqlx migrator runs, [`normalise_migration_checksums`](../../src-tauri/crates/app/src/commands/profile_io.rs) rewrites `_sqlx_migrations.checksum` for every version present in both the archive and the local migrator — older builds checked out migration files with CRLF endings (Windows `core.autocrlf=true` + no `.gitattributes` lock) so their stored SHA-384 differs from the same SQL re-hashed today, even though the DDL is identical. A `.gitattributes` at repo root now pins `*.sql` / `*.rs` / `*.ts` / etc. to LF so future archives stay byte-stable. Once normalised, the new pool is opened once so any pending sqlx migrations replay before the user switches to it. An archive whose `_sqlx_migrations` lists a version unknown to the local migrator is rejected — that means the export came from a newer build.
- **Feedback and failures:** an archive carries the artwork cache, so a large library takes minutes; the card shows an "Exporting…" / "Importing…" line while the buttons are greyed, since greyed buttons alone read as a hang. Both commands log their error at `warn` (`profile export failed` / `profile import failed`) — the UI can only show one generic sentence, and until #767 the cause was written nowhere.
...
[`commands/preferences.rs`](../../src-tauri/crates/app/src/commands/preferences.rs) owns the preferences that have a side effect outside the database — the original release shipped the switches without wiring them, so they reset on every restart:
