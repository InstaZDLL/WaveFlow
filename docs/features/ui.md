...
| Tier         | Controls                                                                                                                                                                                                                | Where                                                                                                                                                                                                                  |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Primary**  | Lyrics, Queue, Device picker, "⋯", Volume, Mini-player, Immersive view, plus any pinned overflow item (A-B loop, Sleep timer, EQ presets). Every entry is opt-out from the user side via Settings → Appearance (defaults match the pre-customisation layout — zero visible change after the upgrade). | Each button reads its visibility from `usePlayerBarLayout` ([`src/hooks/usePlayerBarLayout.ts`](../../src/hooks/usePlayerBarLayout.ts)). The same hook drives the live preview in the Settings panel.                  |
...
| **Pinnable** | A-B loop, Sleep timer, EQ presets (promote each to primary independently)                                                                                                                                               | Settings → Appearance → "Player bar layout" — single panel covering every button + the cover-click action.                                                                                                            |
