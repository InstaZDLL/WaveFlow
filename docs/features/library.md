...
- **Bulk track endpoints wire format** — `list_tracks`, `list_playlist_tracks` and `list_liked_tracks` return `ListTracksResponse { artwork_base, items: TrackListItem[] }` instead of `Track[]`. Each item carries `artwork_hash` + `artwork_format` + `artwork_has_1x` + `artwork_has_2x`; the ~70-char per-profile prefix only appears once in `artwork_base`. [`expandTrackResponse`](../../src/lib/tauri/track.ts) stitches the absolute paths back together so every UI consumer keeps the full `Track` shape unchanged. Cuts ~30 % off the payload (≈ 1.0 MB → ≈ 700 kB on a 1k-track library), proportionally shrinking JSON parse + IPC transfer time. Any future bulk endpoint shipping artworks for hundreds of rows should adopt the same `{ artwork_base, items }` shape.
