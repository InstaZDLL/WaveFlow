...
[`fetch_radio_lyrics`](../../src-tauri/crates/app/src/commands/lyrics.rs) keys a dedicated `radio_lyrics` table in `app.db` by `blake3(artist + title)` parsed from the ICY stream title. A radio session has no library row and no `file_hash`, so it can't use the normal `lyrics` cache. It queries LRCLIB plus the query fallback chain and caches misses as empty rows, same as the library path.
