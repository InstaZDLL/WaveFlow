...
| **Plugins**         | RFC-002 **plugin SDK** — wasmtime sandbox with permission gates, manifest-declared host APIs, settings panel per plugin; ships the first official **Web Radio** plugin (30 000+ radio-browser.info stations) routing live streams through the cpal engine, with per-profile station favorites + country browsing (local-station shortcut + 200+ country picker)                                                                                              | docs/features/plugins.md *(WIP)*         |
