...
def main() -> int:
    ...
    # caller's working copy is out of sync (rebase gone wrong, locale
    # file deleted by accident, repo layout drift). `skipped` covers
    # the benign no-op case where the locale is already at the target
    # values; idempotent re-runs must keep returning 0 so CI / pre-
    # commit hooks can call this script unconditionally.
    missing: list[str] = []
