...
//!
//! The Better Auth handshake against `waveflow-server` spins up a
//! one-shot `tiny_http` server on a local port to capture the redirect
//! from the system browser, then renders a small HTML confirmation
//! page so the user knows it's safe to close the
...
/// renders the raw HTML markup as literal text. That broke the Better
/// Auth confirmation page until this helper landed.
