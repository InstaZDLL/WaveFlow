...
pub enum SmartPlaylistRules {
    ...
    DailyMix {
        slot: u8,
        /// Whether this mix was actually built out of whole records.
        ///
        /// The session outlives its generation: a mix is a stored
        /// playlist, and playing it a week later goes through the
        /// ordinary playlist path, which has no other way to know the
        /// tracks were chosen as records. Without it, automatic
        /// ReplayGain hands a session of whole albums the track gain
        /// (#647).
        ///
        /// **What was produced, not what was asked for.** Album mode is
        /// a preference and the generator falls back to tracks when no
        /// record qualifies, so the flag records the fallback too.
        ///
        /// `default` because the rows written before this field existed
        /// have to keep parsing — and `false` is the right reading of
        /// them: they were generated by a build that only ever chose
        /// tracks.
        #[serde(default)]
        album_mode: bool,
    },
